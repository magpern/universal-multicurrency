...
| `umc_preferred_currency` | `User\PreferredCurrency` | Authenticated shopper's uppercase ISO preferred currency; invalid stored values are retained but fall back effectively to base | **Preserved** (ADR-0036) |
...
| `umc_currency_origin` | `CurrencySwitcher` | Explanatory origin of the persisted shopper currency (`customer`, `visitor_location`, or `user_preference`). **Never** used by `CurrencyResolver` for precedence. Updated on every `persist()` write. |
...
| `umc_preferred_currency` user meta | **Preserved** |
